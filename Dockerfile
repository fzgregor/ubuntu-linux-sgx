...
RUN git clone https://github.com/01org/linux-sgx.git && cd linux-sgx && git reset --hard 6662022b && ./download_prebuilt.sh && make && make sdk_install_pkg && make psw_install_pkg

# copy the patch necessary for in container installation
COPY psw-installer.patch /linux-sgx

# and apply it
RUN patch /linux-sgx/linux/installer/bin/sgx_linux_x64_psw_1.7.100.36470.bin < /linux-sgx/psw-installer.patch

# before installing the sdk and the platform services
RUN echo yes | /linux-sgx/linux/installer/bin/sgx_linux_x64_sdk_1.7.100.36470.bin
RUN /linux-sgx/linux/installer/bin/sgx_linux_x64_psw_1.7.100.36470.bin
